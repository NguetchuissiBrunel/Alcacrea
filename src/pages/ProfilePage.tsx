...
export function ProfilePage() {
  ...
  if (loading || refreshing) {
    return (
      <>
        <Header title={t('profile.title')} subtitle={t('profile.subtitle')} compact />
        <ProfileSkeleton />
      </>
    )
  }
    ...
    <>
      ...
      <div className="rounded-[var(--radius-organic)] surface-card overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(260px,1fr)_2fr] lg:divide-x lg:divide-vellum/8">
          <aside className="flex flex-col p-6 sm:p-8 bg-ink-muted/25 border-b lg:border-b-0 border-vellum/8">
            <div className="flex flex-col items-center text-center flex-1">
              <div className="w-20 h-20 rounded-2xl bg-breath/15 border border-breath/25 flex items-center justify-center shrink-0">
                <span className="font-serif text-3xl text-breath">{initials}</span>
              </div>
              <h2 className="font-serif text-2xl text-vellum mt-5 leading-tight">
                {form.prenom && (
                  <span className="font-sans font-light text-vellum/75">{form.prenom}</span>
                )}
              ...
              <p className="mt-3 text-vellum/50 text-sm break-all">{user.email}</p>
              <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-dream/10 border border-dream/20">
              ...
              <p className="mt-5 text-[10px] font-sans text-vellum/35 uppercase tracking-wider">
          ...
          <div className="p-5 sm:p-8 space-y-8">
            <form onSubmit={handleSave}>
              ...
              <div className="mt-5 p-4 rounded-xl bg-ink border border-vellum/8">
                <p className="text-[10px] font-sans uppercase tracking-wider text-vellum/35 mb-1">{t('common.email')}</p>
                <p className="text-vellum text-sm font-sans break-all">{user.email}</p>
                <p className="mt-1 text-[10px] text-vellum/30">{t('common.emailLocked')}</p>
              </div>
              <Button type="submit" className="mt-5" loading={saving} loadingText={t('common.saving')} icon={<Save className="w-4 h-4" />}>
                {t('common.save')}
              </Button>
            </form>

            <div className="border-t border-vellum/8 pt-8">
              <form onSubmit={handlePasswordSave}>
                ...
                <Button type="submit" className="mt-5" loading={savingPassword} loadingText={t('common.saving')} icon={<Save className="w-4 h-4" />}>
